...
namespace App\Http\Controllers\MasterData\Penilaian;
...
class DataPembelajaranController extends Controller
{
    ...
    public function index()
    {
        if(auth()->user()->role === 'walisiswa' || auth()->user()->role === 'siswa'){
            ...
            $nilai = NilaiPas::select('pembelajaran_id')->union(NilaiPts::select('pembelajaran_id'))->union(NilaiPengetahuan::select('pembelajaran_id'))->union(NilaiKeterampilan::select('pembelajaran_id'))->get();
            $role = auth()->user()->role;
    ...
    public function store(StorePembelajaranRequest $request, $role)
    {
        ...
        $pembelajaran;
        return redirect(route('datapembelajaran.index', $role))
        ->withInfo('Data Pembelajaran: <b>' . $pembelajaran->mapel->singkatan . ' ' . $pembelajaran->kelas->name . '</b> berhasil ditambahkan!');
        }
    }
    ...
    public function insertNilai(Request $request, $id) 
    {
        ...
        foreach ($siswas as $i => $siswa) {
            ...
            // Diperbarui
            $nk = $request->nilai_pengetahuans[$i] * 4;
            $np = $request->nilai_keterampilans[$i] * 5;
            $nt = $request->nilai_pts[$i] * 7;
            $na = $request->nilai_pas[$i] * 9;
            $rataRata = ($nk + $np + $nt + $na) / 25;
