<div class="row">
    <div class="col-md-6">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
                <select class="form-select @error('tingkat') is-invalid @enderror" name="tingkat" id="">
                    ...
                    <option value="7" {{ '7' == old('tingkat', $kelas->tingkat) ? 'selected' : '' }}>7 (Tujuh)
                    </option>
                    <option value="8" {{ '8' == old('tingkat', $kelas->tingkat) ? 'selected' : '' }}>8 (Delapan)
                    </option>
                    <option value="9" {{ '9' == old('tingkat', $kelas->tingkat) ? 'selected' : '' }}>9 (Sembilan)
                    </option>
                </select>
                @error('tingkat')
                    <span class="invalid-feedback mt-1">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
                <select class="form-select @error('guru_id') is-invalid @enderror" name="guru_id" id="exampleSelectBorder">
                    ...
                    <option value="{{ $kelas->guru->id }}" {{ $kelas->guru->id == old('guru_id', $kelas->guru->id) ? 'selected' : '' }}>
                        {{ $kelas->guru->name }}{{ $kelas->guru->gelar ? ', ' . $kelas->guru->gelar : '' }}
                    </option>
                    @foreach ($guru as $item)
                        <option value="{{ $item->id }}" {{ $item->id == old('guru_id', $kelas->guru_id) ? 'selected' : '' }}>
            ...
            <div class="col-sm-8">
                <select class="form-select @error('tapel_id') is-invalid @enderror" name="tapel_id" id="exampleSelectBorder">
                    ...
                    @foreach ($tapel as $item)
                        <option value="{{ $item->id }}" {{ $item->id == old('tapel_id', $kelas->tapel_id) ? 'selected' : '' }}>
                            {{ $item->tahun_pelajaran }} - {{ $item->semester == 1 ? 'Ganjil' : 'Genap' }}
                        </option>
                    @endforeach
                </select>
                @error('tapel_id')
        ...
        <div class="offset-sm-4 col-sm-8 mt-2">
            <a class="btn btn-danger" href="{{ route('datakelas.index', $role) }}">Batal</a>
